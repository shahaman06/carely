...
conn = mysql.connector.connect(
    host="localhost", user="carely_admin", passwd="carely_admin", database="carelydb"
)
cur = conn.cursor()
...
if len(res) > 1:
    ...
    if res[0][1] == "True":
        ...
        if res[0][2] == "True":
            sqlcmd = (
                "update table VerificationProof set under_ver ='False',ver_flag='True' where carely_id = '"
                + user_id
                + "'"
            )
            cur.execute(sqlcmd)
